...
// Add these methods to your dbHelper object in db.js
...
async validateUserPin(fullName, pin) {
  ...
  const inputPinHash = this.hashPin(pin);
  return user.pin_hash === inputPinHash;
},

// Get all users (just the names)
async getUsers() {
...
async getProjectsByUser(fullName) {
  const drafts = await this.getDraftsByUser(fullName);
  const projects = [...new Set(drafts.map(draft => draft.project_name))];
  return projects.filter(project => project && project.trim() !== '');
},

// Create a new project for a user (this creates an initial placeholder draft)
async createProject(fullName, projectName) {
  const db = await this.getDB();
  
  // Check if project already exists for this user
  const existingDrafts = await this.getDraftsByUserAndProject(fullName, projectName);
  if (existingDrafts.length > 0) {
    throw new Error("A project with this name already exists for this user");
  }
  
  // Create a placeholder draft to establish the project
  const placeholderDraft = {
    device_id: this.getDeviceId(),
    full_name: fullName,
    project_name: projectName,
    ideal_customer_profile: 'Not specified',
    product_idea: 'Not specified', 
    pain_points: 'Not specified',
    alternatives: 'Not specified',
    category: [],
    heard_about: '',
    version: 0, // Version 0 indicates this is a placeholder
    is_final: false,
    quality_score: 0,
    created_at: new Date().toISOString(),
  ...
  return this.saveDraft(placeholderDraft);
},

// Enhanced delete user method that removes all associated data
async deleteUserAndData(fullName) {
  const db = await this.getDB();
  
  try {
    // Start a transaction that includes all stores we need to clean
    const transaction = db.transaction(["users", "drafts"], "readwrite");
    const userStore = transaction.objectStore("users");
    const draftStore = transaction.objectStore("drafts");
    const draftIndex = draftStore.index("full_name");
    
    // Delete all drafts for this user
    const drafts = await new Promise((resolve, reject) => {
      const request = draftIndex.getAll(fullName);
      request.onsuccess = () => resolve(request.result || []);
      request.onerror = () => reject(request.error);
    });
    
    // Delete each draft
    const deleteDraftPromises = drafts.map(draft => {
      return new Promise((resolve, reject) => {
        const request = draftStore.delete(draft.id);
    ...
    console.log(`Deleted ${drafts.length} drafts for user ${fullName}`);
    
    // Delete the user
    await new Promise((resolve, reject) => {
      const request = userStore.delete(fullName);
      request.onsuccess = () => resolve(true);
      request.onerror = () => reject(request.error);
    });
    
    console.log(`Successfully deleted user ${fullName} and all associated data`);
    return true;
...
// Helper method to check if a draft is a placeholder
isDraftPlaceholder(draft) {
  if (!draft) return true;
  
  // Version 0 drafts are always placeholders
  if (draft.version === 0) return true;
